...
Transformer由编码器和解码器构成。图2.1展示了该结构，其左侧和右侧分别对应着编码器(Encoder)和解码器(Decoder)结构，它们均由若干个基本的 Transfo rmer Encoder/Decoder Block(N×表示N次堆叠)。
![[Pasted image 20240528102430.png]]
...
假设输入表示包含一个序列中个词元的维嵌入表示。 位置编码使用相同形状的位置嵌入矩阵 输出 ， 矩阵第行、第列和列上的元素为：
![[Pasted image 20240528104726.png]]

 
其中，表示单词所在的位置，和表示位置编码向量中的对应维度， 则对应位置编码的总维度。

通过上面这种方式计算位置编码有这样几个好处：

首先，正余弦函数的范围是在 [-1,+1]，导出的位置编码与原词嵌入相加不会使得结果偏离过远而破坏原有单词的语义信息。

其次，依据三角函数的基本性质，可以得知第个位置的编码是第个位置的编码的线性组合，这就意味着位置编码中蕴含着单词之间的距离信息。
...
考虑到在过大时，点积值较大会使得后续Softmax操作溢出导致梯度爆炸，不利于模型优化。故将注意力得分除以进行缩放。

注：当时，就是传统的注意力机制(1个, 多个,)。
...
masked_softmax函数实现了掩码操作(masked softmax operation)， 其中任何超出有效长度的位置都被掩蔽并置为(将掩码位置的注意力系数变为无穷小，后的值为一个接近的值)
...
当n=m时，且、、均源于输入经过不同的线性变换时，缩放点积注意力即推广为自注意力。
...
原文链接：https://blog.csdn.net/SHIDACSDN/article/details/134030921