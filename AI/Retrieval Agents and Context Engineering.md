![Databricks Learning](https://cdn5.dcbstatic.com/files/d/a/databricks_docebosaas_com/1778893200/bhNCIlUggYvWP3FAlYLVqQ/scorm/f0002a80b36c50d36813618b626030776a0ad6bfc0cf994954555ae48db65648/scormcontent/images/db-academy-rgb-1200px-02306dce88.png)
...
1. **The Knowledge Cutoff:** The model cannot answer questions about events that occurred after the cutoff date of its training data.
    ...
    - _Example Prompt:_ `Who won the 2025 Nobel Prize in Physics?`
...
- **Generation:** The model synthesizes an answer using _only_ the injected data
...
- **Negative Constraints:** Define what the model _cannot_ do (e.g., "Do not mention competitor products" or "Do not provide code unless explicitly requested")
...
- **Grounding Instructions:** Use explicit instructions to bind the model to the retrieved context. For example: _"Answer using ONLY the provided context chunks. If the answer is not present, state 'I do not have that information.'"_
- **Metadata Filtering:** Utilize **Unity Catalog** metadata to filter retrieval _before_ it reaches the model. For example, if a user asks about "2024 Revenue," the system should filter chunks where `year=2024` to prevent the model from seeing outdated 2023 data
...
3. **Grounding is Mandatory:** Strictly instruct the model to use _only_ retrieved data and leverage **Unity Catalog** metadata to ensure that data is relevant and secure.

© 2026 Databricks, Inc. All rights reserved. Apache, Apache Spark, Spark, the Spark Logo, Apache Iceberg, Iceberg, and the Apache Iceberg logo are trademarks of the [Apache Software Foundation](https://www.apache.org/).  
...
![Databricks Learning](https://cdn5.dcbstatic.com/files/d/a/databricks_docebosaas_com/1778900400/27MAsy8ptdMHAiISUntYWg/scorm/80237032fe465c13509059e87bdf476a37c262820ff86ecb037e4eb999d6839a/scormcontent/images/db-academy-rgb-1200px-02306dce88.png)
...
![Data storage, ingestion and processing workflow](https://cdn5.dcbstatic.com/files/d/a/databricks_docebosaas_com/1778900400/27MAsy8ptdMHAiISUntYWg/scorm/80237032fe465c13509059e87bdf476a37c262820ff86ecb037e4eb999d6839a/scormcontent/images/02-process-diagram-0e52bcbb59.png)

_Figure 1. This diagram show 5 main steps of data ingestion, processing and embedding generation workflow._
...
![An example page showing complex page layout](https://cdn5.dcbstatic.com/files/d/a/databricks_docebosaas_com/1778900400/27MAsy8ptdMHAiISUntYWg/scorm/80237032fe465c13509059e87bdf476a37c262820ff86ecb037e4eb999d6839a/scormcontent/images/02-complex-page-structure-example-61162ede2a.png)

_Figure 2. This image shows an example page layout which has complex page layout_
...
![Chunking methods visualized](https://cdn5.dcbstatic.com/files/d/a/databricks_docebosaas_com/1778900400/27MAsy8ptdMHAiISUntYWg/scorm/80237032fe465c13509059e87bdf476a37c262820ff86ecb037e4eb999d6839a/scormcontent/images/02-chunking-methods-d5a0589c1c.png)

_Figure 3. A visual representation of fixed-size chunking and semantic chunking._
...
![Advanced chunking methods visualized](https://cdn5.dcbstatic.com/files/d/a/databricks_docebosaas_com/1778900400/27MAsy8ptdMHAiISUntYWg/scorm/80237032fe465c13509059e87bdf476a37c262820ff86ecb037e4eb999d6839a/scormcontent/images/02-advanced-chunking-methods-9f045c76b7.png)

_Figure 4. A visual representation of advanced chunking methods. Each color represents a chunk._

### D3. Embedding Model Considerations

While embedding models are covered in detail in a later module, their technical constraints must be considered _now_ during the chunking phase.
...
2. **Chunking (Splitting):** After extraction, the text must be divided into smaller, manageable chunks.
